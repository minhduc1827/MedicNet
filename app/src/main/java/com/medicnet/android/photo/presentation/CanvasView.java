...
package com.medicnet.android.photo.presentation;
...
public class CanvasView extends View {
    ...
    private void drawText(Canvas canvas) {
        ...
        int numChars = (lengthOfChar <= 0) ? 1 : (int) Math.floor((double) (restWidth /
                lengthOfChar));  // The number of characters at 1 line
    ...
    private void onActionDown(MotionEvent event) {
        switch (this.mode) {
            case DRAW:
            case ERASER:
                if ((this.drawer != Drawer.QUADRATIC_BEZIER) && (this.drawer != Drawer
                        .QUBIC_BEZIER)) {
                    // Oherwise
                ...
                if ((this.drawer != Drawer.QUADRATIC_BEZIER) && (this.drawer != Drawer
                    ...
                    switch (this.drawer) {
                        ...
                        case CIRCLE:
                            ...
                            double radius = Math.sqrt(Math.pow(distanceX, 2.0) + Math.pow
                                    (distanceY, 2.0));

                            path.reset();
                            path.addCircle(this.startX, this.startY, (float) radius, Path
                                    .Direction.CCW);
                            break;
                        case ELLIPSE:
...
//            canvas.drawBitmap(this.bitmap, 0F, 0F, emptyPaint);
            canvas.drawBitmap(this.bitmap, null, new Rect(0, 0, width, height), emptyPaint);
        }
